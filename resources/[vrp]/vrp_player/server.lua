...
-- WEBHOOK
-----------------------------------------------------------------------------------------------------------------------------------------
local webhookgarmas = "https://discord.com/api/webhooks/774281932547555408/PDunaEhqvdMEbFTu2A5cRK-BSZmP2TPpA3rBpFWotJeRvMEMomeZU9rKKn9TAUjd5H9h"
local webhookenviardinheiro = "https://discordapp.com/api/webhooks/768778071696211979/JEoj9-NtZVSWe2zHPTQXUIkDNeMc6zc91wxFetDhrtYE_Xmk4qGKuKI0t5kqyiI4Wzzy"
local webhookpaypal = "https://discordapp.com/api/webhooks/768778197336588298/vFiZmTsIsdi0ihqxI21NT-3lSXCHa_pY5EYMklnCrc3gDbMVNdwA-P_lRJ3qwFM3OdXO"
local webhooksaquear = "https://discordapp.com/api/webhooks/768778288264904705/T96zIRAEedoZEagM1zrm4HNMage60dbMNG15uAgAitFAAhS7YZrpdCG7oR_BFBnwkHks"
...
function src.checkVIP()
	...
	if user_id then
		if vRP.hasPermission(user_id,"last.permissao") or vRP.hasPermission(user_id,"ultimate.permissao") or vRP.hasPermission(user_id,"hire.permissao") or vRP.hasPermission(user_id,"booster.permissao") or vRP.hasPermission(user_id,"suporte.permissao") then
			return true
		else
...
function src.checkAttachs()
	...
	local user_id = vRP.getUserId(source)
	if user_id then
		if vRP.getInventoryItemAmount(user_id,"compattach") >= 1 or vRP.hasPermission(user_id,"ultimate.permissao")  or vRP.hasPermission(user_id,"last.permissao") or vRP.hasPermission(user_id,"suporte.permissao") then
			return true
		else
...
function src.checkRoupas()
	...
	local user_id = vRP.getUserId(source)
	if user_id then
		if vRP.getInventoryItemAmount(user_id,"roupas") >= 1 or vRP.hasPermission(user_id,"ultimate.permissao") or vRP.hasPermission(user_id,"suporte.permissao") then
			return true 
		else
...
-----------------------------------------------------------------------------------------------------------------------------------------
local salarios = {
	{ ['permissao'] = "hire.permissao", ['nome'] = "HIRE", ['payment'] = 850 },
	{ ['permissao'] = "last.permissao", ['nome'] = "LAST", ['payment'] = 1000 },
    { ['permissao'] = "ultimate.permissao", ['nome'] = "ULTIMATE", ['payment'] = 2000 },
...
RegisterCommand('coc',function(source,args,rawCommand)
	if args[1] then
		...
		if vRP.hasPermission(user_id,"conce.permissao") then
			if user_id then
				TriggerClientEvent('chatMessage',-1,"Concessionaria | "..identity.name.." "..identity.firstname.." ("..user_id..") ",{176,196,222},rawCommand:sub(4))
			end
		end
	end
end)
...
RegisterCommand('st',function(source,args,rawCommand)
	if args[1] then
		...
		if vRP.hasPermission(user_id,permission) then
			local adm = vRP.getUsersByPermission(permission)
			for l,w in pairs(adm) do
				local player = vRP.getUserSource(parseInt(w))
				if player then
					async(function()
						TriggerClientEvent('chatMessage',player,"ADMINISTRAÇÃO "..identity.name.." "..identity.firstname.." ("..user_id..") ",{255,0,0},rawCommand:sub(3))
					end)
				end
			end
		end
