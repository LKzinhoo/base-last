...
rsh = {}
Tunnel.bindInterface("rsh_modoadm",rsh)
Proxy.addInterface("rsh_modoadm",rsh)
rsh = Tunnel.getInterface("rsh_modoadm")
...
Citizen.CreateThread(function()
	while true do
	    for _, id in ipairs(GetActivePlayers()) do
			...
			local pid = rsh.getId(GetPlayerServerId(id))
			if pid == -1 then
				return
			end
...
RegisterCommand("am",function()
	...
	if showIds then
		rsh.reportLog("ADMIN MODE ON")
		print("ADMIN MODE ON")
	else
		rsh.reportLog("ADMIN MODE OFF")
